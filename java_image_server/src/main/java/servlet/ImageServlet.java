...
public class ImageServlet extends HttpServlet {
    public static final String PATH_BASE = "/root/project/image_system/";
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        image.setPath("./image/" + image.getMd5());//重名文件会失败
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        //  b) 删除磁盘文件, 此时说明已经不存在相同 MD5 的图片了, 可以删除磁盘上的图片文件 防止相同图片删除后
        //影响其他的图片
        Image existImage = imageDao.selectByMD5(image.getMd5());
        if (existImage == null) {
            File file = new File(image.getPath());
