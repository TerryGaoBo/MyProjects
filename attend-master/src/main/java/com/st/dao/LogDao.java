...
public class LogDao {
     ...
     * @return
     */
    public List<Log> getByWorkdateAndUid(String workdate,int uid) {
        List<Log> list = new ArrayList<Log>();
        DBUtil db = new DBUtil();
        String sql = "select id,`describe`,worktime,difficulty,remark,status from log where uid=? and workdate=? order by workdate desc";
        Object[] params = {uid,workdate};
        Date date = Date.valueOf(workdate);
        try {
            ResultSet rs = db.doQuery(sql, params);
            while (rs.next()) {
                int id = rs.getInt(1);
                String describe = rs.getString(2);
                int worktime = rs.getInt(3);
                String diffculty = rs.getString(4);
                String remark = rs.getString(5);
                String status = rs.getString(6);
                Log log = new Log(id,uid,date,describe,worktime,diffculty,remark,status);
                list.add(log);
            }
            return list;
        }catch (Exception e){
     ...
     * @return
     */
    public List<Log> getByWorkdate(String workdate) {
        List<Log> list = new ArrayList<Log>();
        DBUtil db = new DBUtil();
        String sql = "select id,uid,`describe`,worktime,difficulty,remark,status from log where workdate=? order by workdate desc";
        Object[] params = {workdate};
        Date date = Date.valueOf(workdate);
        try {
            ResultSet rs = db.doQuery(sql, params);
            while (rs.next()) {
    ...
    public boolean queryLogBuyUid(int uid) {
        DBUtil db = new DBUtil();
    ...
    public boolean update(Log log){
        DBUtil db = new DBUtil();
        String sql = "update Log set workdate=?,`describe`=?,worktime=?,difficulty=?,remark=?,`status`=? where id =?";
        Object[] params = {
                log.getWorkdate(),
                log.getDesribe(),
                log.getWorktime(),
